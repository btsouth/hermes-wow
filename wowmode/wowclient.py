...
OUTBOX_KINDS = {"reply", "focus", "mark_read", "stop"}
...
def publish(
    ...
    *,
    ...
    notice = str(dispatch_state.get("control_error") or "")
    ...
    try:
        ...
        for item in pending.values():
            ...
            if any(not isinstance(item.get(field, ""), str) for field in
                   ("seq", "kind", "host", "session_id", "text", "log", "completion")):
...
def dispatch(
    ...
    *,
    ...
    if state.get("unreadable"):
        ...
        state["control_error"] = "Dispatch ledger was unreadable; queued actions skipped. Inspect Hermes before resending."
    ...
    for entry in entries:
        ...
        if entry.get("kind") == "focus" or (entry.get("legacy") and entry.get("text", "").strip() == "!focus"):
            ...
            state["control_error"] = "Stop delivery uncertain; inspect Hermes before trying again"
            _save_state(state)
            try:
                ...
                outcome = f"stop_failed_or_uncertain: {exc}; inspect Hermes before trying again"
