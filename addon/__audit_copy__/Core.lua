...
-- Default positions, named because three places need them (the two frames and
-- the reset button) and a fourth copy is how they drift apart. The badge sits
-- BELOW the minimap cluster, which is where a 190x30 frame at TOPRIGHT covers
-- the minimap itself.
...
function ns:QueueEntry(kind, sessionId, text, host)
  ...
  HermesAIDB.seq = (tonumber(HermesAIDB.seq) or 0) + 1
...
function ns:TrimOutbox(acked)
  ...
  for _, entry in ipairs(self:OutboxEntries()) do
    local seq = tonumber(string.match(entry, "^(%d+)"))
    if seq and seq <= acked then
      dropped = dropped + 1
    else
      kept[#kept + 1] = entry
    end
...
events:SetScript("OnEvent", function(_, event, arg1, arg2)
  ...
  if event == "PLAYER_LOGIN" then
    ...
    if loaded.missing then
      ns:Print(L["no snapshot yet: run hermes-wow wow install on this machine, then sync."])
      return
    end
...
function ns:Summary()
  ...
  for _, status in ipairs(self.STATUS_ORDER) do
    ...
    if count > 0 then
      table.insert(parts, count .. " " .. string.lower(self:StatusLabel(status)))
    end
  end
...
COMMANDS = {
  ...
  },
  {
    verb = "settings",
    args = "",
    help = "Open or close the settings pane",
    run = function()
  ...
  },
  {
    verb = "sync",
    args = "",
    help = "Send what is queued and pull a fresh snapshot (a UI reload)",
    run = function()
      ns:Sync("slash")
    end,
  },
  { verb = "status", aliases = { "who" }, args = "", help = "What the snapshot holds", run = printStatus },
  {
    verb = "tab",
    args = "[name]",
    help = "Switch tab (all, needs, working, waiting, reply, finished)",
    run = function(rest)
  ...
  },
  {
    ...
    aliases = { "find" },
    args = "[text]",
    help = "Filter the rows (empty text clears it)",
    run = function(rest)
  ...
  },
  {
    ...
    help = "Queue a reply to one session",
    run = function(rest)
  ...
  },
  {
    ...
    aliases = { "focus", "open" },
    args = "<session id>",
    help = "Put a session on the clipboard for the desktop app",
    run = function(rest)
  ...
  },
  {
    verb = "hosts",
    args = "",
    help = "Which machines answered the last publish",
    run = function()
      local names = {}
  ...
  },
  {
    ...
    help = "How eagerly an unprompted sync may spend a reload",
    run = function(rest)
  ...
  },
  {
    ...
    aliases = { "autosync" },
    args = "",
    help = "Toggle refreshing on loading screens",
    run = function()
  ...
  },
  {
    verb = "sendonsync",
    args = "",
    help = "Toggle sending queued replies as soon as you send one",
    run = function()
      toggleFlag("sendOnSync", L["send on sync"])
    end,
  },
  { verb = "sound", args = "", help = "Toggle the attention sound", run = function() toggleFlag("sound", L["Sound"]) end },
  {
    verb = "minimap",
    args = "",
  ...
  },
  {
    verb = "theme",
    args = "",
    help = "Switch between the Hermes skin and the plain one",
    run = function()
